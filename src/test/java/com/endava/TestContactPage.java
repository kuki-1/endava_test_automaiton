...
class TestContactPage {
	...
	/**
	 * @author Vladimir Krekic From Burger menu go to Contact page, validate new URL
	 *         is correct, validate both radio buttons ("Interested in our
	 *         services?" and "Want to join our team?") are not selected, click on
	 *         "Want to join our team? " Radio button, validate it's selected
	 *         Validate message "Please visit the Careers section on our website to
	 *         apply for job openings. Please use the form below if you have another
	 *         question or important message." appears.
	 */
	@Test
	public void testContactPage() {
		...
		Assert.assertEquals(ContactPage.getContactUrl(), contactsPage.driver.getCurrentUrl(),
				"ContactPage Url does not mach");
		...
		Assert.assertFalse(
				contactsPage.getSearchResult(contactsPage.driver.findElement(contactsPage.getJoinMessage())).isEmpty(),
				"Element \"JoinRadioButton\" not selected");
		Assert.assertEquals(contactsPage.getMessage(),
				contactsPage.getSearchResult(contactsPage.driver.findElement(contactsPage.getJoinMessage())),
